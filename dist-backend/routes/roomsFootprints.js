...
async function getLatestChangeset(accessToken, iModelId) {
    const headers = {
    ...
    };
    {
        ...
        if (res.ok) {
            ...
            if (cs && typeof cs.id === "string" && typeof cs.index === "number")
                return { id: cs.id, index: cs.index };
        }
    }
    {
        ...
        for (const c of changesets) {
            if (typeof c.id !== "string" || typeof c.index !== "number")
                continue;
            if (!best || c.index > best.index)
                best = { id: c.id, index: c.index };
        }
        if (!best)
...
function mergeZ(debug, zMin, zMax) {
    if (typeof zMin === "number")
        debug.zMin = typeof debug.zMin === "number" ? Math.min(debug.zMin, zMin) : zMin;
    if (typeof zMax === "number")
        debug.zMax = typeof debug.zMax === "number" ? Math.max(debug.zMax, zMax) : zMax;
}
function polyfaceFacetAndZStats(pf) {
...
function pointKey(p, precision = 1e6) {
...
function closeLoopIfNeeded(loop) {
    if (loop.length < 2)
        return loop;
    const a = loop[0];
...
function polyfaceTopBoundaryLoops(pf) {
    ...
    for (visitor.reset(); visitor.moveToNextFacet();) {
        ...
        if (n < 3)
            continue;
        const p0 = Point3d.create(pts3d.getXAtUncheckedPointIndex(0), pts3d.getYAtUncheckedPointIndex(0), pts3d.getZAtUncheckedPointIndex(0));
        const p1 = Point3d.create(pts3d.getXAtUncheckedPointIndex(1), pts3d.getYAtUncheckedPointIndex(1), pts3d.getZAtUncheckedPointIndex(1));
        const p2 = Point3d.create(pts3d.getXAtUncheckedPointIndex(2), pts3d.getYAtUncheckedPointIndex(2), pts3d.getZAtUncheckedPointIndex(2));
        ...
        if (Math.abs(dotUp) < 0.05)
            continue;
        let zSum = 0;
        for (let i = 0; i < n; i++)
            zSum += pts3d.getZAtUncheckedPointIndex(i);
        const zAvg = zSum / n;
        if (Number.isFinite(maxZ) && (maxZ - zAvg) > zTol)
            continue;
        const facetKeys = [];
        for (let i = 0; i < n; i++) {
    ...
    for (const [k, e] of edgeCounts.entries()) {
        ...
        const add = (u, v) => {
            const set = adjacency.get(u) ?? new Set();
            set.add(v);
            adjacency.set(u, set);
        };
        add(e.a, e.b);
    ...
    while (unusedEdges.size > 0) {
        ...
        while (true) {
            ...
            if (ringKeys.length > 10000)
                break;
        }
        const closed = current === start;
        if (!closed)
            continue;
        const ring = ringKeys.map((k) => vertices.get(k)).filter((p) => !!p);
...
function curveCollectionToXYLoops(cc, out) {
    const loops = [];
    const options = StrokeOptions.createForCurves();
    const stroked = cc.cloneStroked(options);
    const closeTol = 1e-4;
    const pointsFromCurvePrimitive = (prim) => {
    ...
    const normalizeRing = (ring) => {
        ...
        for (const p of ring) {
            ...
            if (!last || !p.isAlmostEqual(last, closeTol))
                out.push(p);
        }
        if (out.length >= 2 && out[0].isAlmostEqual(out[out.length - 1], closeTol))
            out.pop();
        return out;
    };
    ...
    const visit = (node) => {
        ...
        if (node instanceof Loop) {
            ...
            for (const child of node.children ?? [])
                segments.push(pointsFromCurvePrimitive(child));
    ...
    if (loops.length === 0) {
        const collectLineStrings = (node, outLs) => {
            if (!node || typeof node !== "object")
                return;
            if (node instanceof LineString3d) {
                outLs.push(node);
        ...
        const addEdge = (a, b) => {
            ...
            if (existing)
                ...
                edgeCounts.set(k, { a: ka, b: kb, count: 1 });
        };
...
function geometryQueryToXYLoops(gq, localToWorld, debug) {
    if (gq instanceof CurveCollection) {
        if (localToWorld) {
            const maybeTransformable = gq;
            if (typeof maybeTransformable.cloneTransformed === "function") {
                ...
                if (transformed instanceof CurveCollection) {
                    if (debug)
                        debug.methods.curveCollection = (debug.methods.curveCollection ?? 0) + 1;
                    const flag = {};
                    ...
                    if (loops.length === 0 && debug)
                        debug.methods.curveCollection0 = (debug.methods.curveCollection0 ?? 0) + 1;
                    return loops;
                }
            }
        }
    ...
    try {
        ...
        if (debug)
            debug.methods.polyface = (debug.methods.polyface ?? 0) + 1;
    }
    catch {
        if (debug)
            debug.methods.polyfaceError = (debug.methods.polyfaceError ?? 0) + 1;
        return [];
    }
    if (localToWorld)
        pf.tryTransformInPlace(localToWorld);
    if (debug?.polyface) {
...
function computeFootprintsWithDebug(iModel, elementId) {
    const loops = [];
    const debug = {
        elementId,
        primitives: 0,
    ...
    const elementPropsUnknown = iModel.elements.getElementProps({ id: elementId, wantGeometry: true });
    if (!elementPropsUnknown || typeof elementPropsUnknown !== "object") {
        debug.reason = "elementProps-missing";
        return { loops, debug };
    }
    const elementProps = elementPropsUnknown;
    if (typeof elementProps.classFullName === "string")
        debug.elementClass = elementProps.classFullName;
    ...
    const stack = [];
    try {
        stack.push(GeometryStreamIterator.fromGeometricElement3d(elementProps));
    }
    catch {
        try {
            stack.push(GeometryStreamIterator.fromGeometricElement2d(elementProps));
        }
        catch {
    ...
    while (stack.length > 0) {
        ...
        for (const entry of it) {
            ...
            if (type === "geometryQuery" && p.geometry) {
                debug.primitives += 1;
                if (entry.localToWorld)
                    debug.transforms.withLocalToWorld += 1;
                else
                    debug.transforms.withoutLocalToWorld += 1;
                ...
                if (dbgState.usedFallbackBoundary)
                    debug.usedFallbackBoundary = true;
                for (const ring of partLoops)
                    loops.push(ring.map((pt) => ({ x: pt.x, y: pt.y })));
                ...
                const partProps = iModel.elements.getElementProps({ id: partId, wantGeometry: true });
                if (partProps?.geom) {
    ...
    if (debug.polyface) {
        ...
        if (debug.polyface.builds === 0)
            debug.polyface = undefined;
    }
    if (!debug.reason) {
        if (debug.primitives === 0)
            debug.reason = "no-geometryQuery-primitives";
        else if (debug.loops === 0) {
            if ((debug.methods.curveCollection ?? 0) > 0 && (debug.methods.curveCollection0 ?? 0) > 0)
                debug.reason = "curveCollection-no-loops";
            else if ((debug.methods.silhouette ?? 0) > 0 && (debug.methods.silhouette0 ?? 0) > 0)
                debug.reason = "silhouette-no-loops";
            else if ((debug.methods.polyfaceError ?? 0) > 0)
                debug.reason = "polyface-error";
            else
...
function computeFootprints(iModel, elementId) {
    ...
    while (stack.length > 0) {
        ...
        for (const entry of it) {
            ...
            const type = p.type;
            if (type === "geometryQuery" && p.geometry) {
                const partLoops = geometryQueryToXYLoops(p.geometry, entry.localToWorld);
                for (const ring of partLoops)
                    loops.push(ring.map((p) => ({ x: p.x, y: p.y })));
            }
            else if (type === "partReference") {
                const part = p.part;
                const partId = part?.id;
...
roomsFootprintsRouter.post("/footprints", async (req, res) => {
    let db;
    try {
        const { iTwinId, iModelId, elementIds, debug } = req.body;
...
//# sourceMappingURL=roomsFootprints.js.map