...
function curveCollectionToXYLoops(cc) {
    const loops = [];
    ...
    const visit = (node) => {
        ...
        if (node instanceof Loop) {
            ...
            for (const child of node.children ?? []) {
                ring.push(...pointsFromCurvePrimitive(child));
            }
            pushIfLoopish(ring);
            return;
        }
        if (node instanceof Path) {
            ...
            const isClosed = ring.length >= 2 && ring[0].isAlmostEqual(ring[ring.length - 1], closeTol);
            if (isClosed)
                pushIfLoopish(ring);
            return;
        }
...
function geometryQueryToXYLoops(gq, localToWorld, debug) {
        ...
        if (localToWorld) {
            ...
            if (typeof maybeTransformable.cloneTransformed === "function") {
                ...
                if (transformed instanceof CurveCollection) {
                    ...
                    return curveCollectionToXYLoops(transformed);
                }
            }
        }
        if (debug)
            debug.methods.curveCollection = (debug.methods.curveCollection ?? 0) + 1;
        return curveCollectionToXYLoops(gq);
    }
    const builder = PolyfaceBuilder.create();
    builder.addGeometryQuery(gq);
    const pf = builder.claimPolyface(true);
    if (localToWorld)
    ...
    if (cc) {
        ...
        const loops = curveCollectionToXYLoops(cc);
        if (loops.length > 0)
            return loops;
    }
    if (debug)
    ...
    while (stack.length > 0) {
        ...
        for (const entry of it) {
            ...
            if (type === "geometryQuery" && p.geometry) {
                debug.primitives += 1;
                const dbgState = { methods: debug.methods, usedFallbackBoundary: false };
                const partLoops = geometryQueryToXYLoops(p.geometry, entry.localToWorld, dbgState);
...
//# sourceMappingURL=roomsFootprints.clean.js.map