...
export async function ensureIModelHostStarted() {
    ...
    const findCachedCheckpointFile = (iModelId, changesetIndex, changesetId) => {
        const tmp = os.tmpdir();
        ...
        try {
            ...
            for (const f of files) {
                if (!f.startsWith(prefix) || !f.endsWith(".bim"))
                    continue;
                ...
                if (!best || idx > best.index)
                    best = { index: idx, file: path.join(tmp, f) };
            }
            return best?.file;
        }
        catch {
    ...
    V2CheckpointManager[_mockCheckpoint] = {
        mockAttach: (checkpoint) => {
            ...
            if (!accessToken || !iModelId)
                throw new Error("Missing accessToken/iModelId for checkpoint mock");
            ...
            return (findCachedCheckpointFile(iModelId, changesetIndex, changesetId) ??
        ...
        mockDownload: (request) => {
            ...
            if (!accessToken || !iModelId || !localFile)
                throw new Error("Invalid checkpoint mock download request");
            ...
            if (!cachedPath)
                throw new Error("Checkpoint file not available yet");
...
export function registerSnapshotKey(fileKey, filePath) {
