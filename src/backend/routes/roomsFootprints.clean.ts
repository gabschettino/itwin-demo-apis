...
function curveCollectionToXYLoops(cc: CurveCollection): Point2d[][] {
  const loops: Point2d[][] = [];
  ...
  const visit = (node: unknown) => {
    ...
    if (node instanceof Path) {
      ...
      if (isClosed) pushIfLoopish(ring);
      return;
    }
...
function geometryQueryToXYLoops(
  ...
  debug?: { methods: Record<string, number>; usedFallbackBoundary: boolean }
): Point2d[][] {
...
function computeFootprintsWithDebug(iModel: IModelDb, elementId: string): { loops: Point2[][]; debug: FootprintDebug } {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of it) {
      ...
      if (type === "geometryQuery" && p.geometry) {
        ...
        const partLoops = geometryQueryToXYLoops(p.geometry as GeometryQuery, entry.localToWorld, dbgState);
  ...
  if (!debug.reason) {
    ...
    else if (debug.loops === 0) debug.reason = "no-loops-extracted";
  }
